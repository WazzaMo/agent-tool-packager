...
const PROVIDER_LABEL = "ClaudeAgentProvider";
const SETTINGS_JSON = "settings.json";
const PROJECT_MCP_JSON = ".mcp.json";
const USER_CLAUDE_JSON = ".claude.json";

const PROTECTED_REMOVE_PATHS = new Set<string>([
  SETTINGS_JSON,
  PROJECT_MCP_JSON,
  USER_CLAUDE_JSON,
]);
...
/**
 * Builds a single MCP JSON merge action for `.mcp.json` (project) or `~/.claude.json` (user layer).
 *
  ...
  return [
    {
      ...
      provenance: provenanceForFragment(packageName, packageVersion, part, mcpFile),
...
/**
 * Builds hook actions: `hooks.json` merges into layer `settings.json`; other hook files copy as raw files.
 *
...
function planNonSkillAsset(
  ...
  if (asset.type === "mcp") {
    return actionsForMcpAsset(ctx, part, asset, src, packageName, packageVersion);
  }
  if (asset.type === "hook") {
...
export class ClaudeAgentProvider implements AgentProvider {
   ...
   * @param _merge - Reserved for structured merge policy (unused for Claude v1 paths).
   ...
   *
   * @param plan - Plan from {@link ClaudeAgentProvider.planInstall}.
...
/**
 * Factory for {@link ClaudeAgentProvider} (same args as constructor).
 *
