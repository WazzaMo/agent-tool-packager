/**
 * Integration: skill install uses Agent Skills directory under .gemini/ (dist CLI + GeminiAgentProvider).
 */
...
describe("Integration: GeminiAgentProvider skill layout", () => {
  let base: string;
...
`
  ...
  it("installs skill.yaml + skill.md bundle under .gemini/skills/{name}/", () => {
    ...
    initPackage(pkgDir, stationDir, {
      type: "skill",
      name: "gem-cap-skill-bundle",
      version: "0.4.0",
      usage: "Gemini skill layout integration",
    ...
    const out = runAtp(["install", "gem-cap-skill-bundle", "--project"], {
      cwd: projectDir,
      env: { STATION_PATH: stationDir },
    });
    expect(out).toContain("Installed gem-cap-skill-bundle");

    const skillMd = path.join(projectDir, ".gemini", "skills", "pdf-kit", "SKILL.md");
    expect(fs.existsSync(skillMd)).toBe(true);
    const text = fs.readFileSync(skillMd, "utf8");
    expect(text).toContain("name: pdf-kit");
    expect(text).toContain("scripts/extract.py");

    const helper = path.join(projectDir, ".gemini", "skills", "pdf-kit", "helper.py");
