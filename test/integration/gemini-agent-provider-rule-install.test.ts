/**
 * Integration: rule-only package install uses GeminiAgentProvider path (dist CLI + temp project).
 */
...
`
  ...
  it("gate is true for catalog rule-only + gemini project install context", () => {
    ...
    initPackage(pkgDir, stationDir, {
      type: "rule",
      name: "gem-cap-rule-pkg",
      version: "0.3.0",
      usage: "Gemini provider routing test",
      components: ["r.md"],
      catalogAdd: true,
    });
    ...
    const stationPkg = path.join(stationDir, "user_packages", "gem-cap-rule-pkg");
    const agentBase = path.join(projectDir, ".gemini");
    const manifest = {
      name: "gem-cap-rule-pkg",
      version: "0.3.0",
      type: "Rule",
    ...
    expect(
      usesGeminiAgentProviderProjectInstall(providerCtx, manifest, {
        promptScope: "project",
  ...
  it("atp install places rule under .gemini/rules/ (GeminiAgentProvider)", () => {
    const ruleContent = "# GeminiAgentProvider e2e\nok\n";
    fs.writeFileSync(path.join(pkgDir, "rule-one.md"), ruleContent);

    initPackage(pkgDir, stationDir, {
      type: "rule",
      name: "gem-cap-e2e-rules",
      version: "1.0.0",
      usage: "E2E Gemini provider",
    ...
    const out = runAtp(["install", "gem-cap-e2e-rules", "--project"], {
      cwd: projectDir,
      env: { STATION_PATH: stationDir },
    });
    expect(out).toContain("Installed gem-cap-e2e-rules");
